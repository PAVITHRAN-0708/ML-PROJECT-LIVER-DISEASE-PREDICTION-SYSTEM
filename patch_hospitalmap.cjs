...
export default function HospitalMap() {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 180px)', minHeight: '500px', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e1e7ec', background: '#f8fafc' }}>
      {quotaExceeded && (
        <div className="bg-amber-50 border-b border-amber-200 text-amber-900 px-4 py-2.5 text-xs md:text-sm text-center sticky top-0 z-50 shadow-sm" style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1000, background: '#fffbeb', color: '#78350f', padding: '10px', fontSize: '14px', borderBottom: '1px solid #fde68a' }}>
          <span>
  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', bottom: '20px', left: '20px', right: '20px', zIndex: 10 }}>
        {!userLoc || locating ? (
          <div style={{ background: 'white', padding: '16px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '4px', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
            {hospitals.map((h, i) => (
              <div 
                key={i} 
                onClick={() => setSelectedHospital(h)}
                style={{ 
                  flex: '0 0 280px', 
                  border: selectedHospital === h ? '2px solid #005f56' : '1px solid transparent', 
                  borderRadius: '10px', 
                  padding: '16px', 
                  cursor: 'pointer',
                  background: 'white',
                  boxShadow: selectedHospital === h ? '0 8px 24px rgba(0,95,86,0.15)' : '0 4px 12px rgba(0,0,0,0.1)',
                  transition: 'all 0.2s ease'
                }}
              >
                <div style={{ fontWeight: 600, fontSize: '16px', color: '#1a1d20', marginBottom: '6px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{h.displayName}</div>
                <div style={{ fontSize: '14px', color: '#4a5568', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{h.formattedAddress}</div>
